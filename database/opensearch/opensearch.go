...
func (d *OpenSearch) Run(migration io.Reader) error {
	...
	if err != nil {
		return &database.Error{OrigErr: err, Err: "failed to read migration"}
	}

	// Split into lines and find the first non-empty line for the action
	...
	parts := strings.Fields(string(bytes.TrimSpace(actionLine)))
	if len(parts) != 2 {
		return ErrInvalidAction
	}
	method, endpoint := strings.ToUpper(parts[0]), parts[1]

	// Everything after that line is the raw body/template
	rawBody := bytes.Join(lines[bodyStart:], []byte("\n"))

	var reqBody []byte

	if strings.HasPrefix(endpoint, "/_scripts/") {
		escaped := strconv.Quote(string(rawBody))
		buf := &bytes.Buffer{}
		buf.WriteString(`{"script":{"lang":"mustache","source":`)
	...
	req, err := http.NewRequest(method, endpoint, bytes.NewReader(reqBody))
	if err != nil {
	...
	if err != nil {
		return &database.Error{OrigErr: err, Err: fmt.Sprintf("request failed: %v", err)}
	}
	defer func() {
		if err := res.Body.Close(); err != nil {
			fmt.Printf("failed to close response body: %v\n", err)
